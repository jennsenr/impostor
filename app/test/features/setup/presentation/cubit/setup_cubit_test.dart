...
void main() {
  ...
  setUp(() {
    ...
    when(() => mockPrefs.getString('pref_player_name')).thenReturn('TestPlayer');
    when(() => mockPrefs.getBool('pref_junior_mode')).thenReturn(false);
    when(() => mockPrefs.getStringList('pref_selected_category_ids')).thenReturn(['animals']);
  ...
  group('SetupCubit', () {
    test('init loads categories and properties from SharedPreferences', () async {
      await Future.delayed(Duration.zero); // allow async loadCategories to finish
    ...
    blocTest<SetupCubit, SetupState>(
      ...
      expect: () => [
        isA<SetupState>().having((state) => state.playerName, 'playerName', 'NewName'),
      ],
      verify: (_) {
         verify(() => mockPrefs.setString('pref_player_name', 'NewName')).called(1);
      },
    );
    ...
    blocTest<SetupCubit, SetupState>(
      ...
      build: () {
        when(() => mockRepo.joinGame(
          gameId: any(named: 'gameId'),
          playerName: any(named: 'playerName'),
          avatarId: any(named: 'avatarId'),
        )).thenAnswer((_) async => JoinResponse(
             game: Game(id: 'g1', code: '1234', status: GameStatus.waiting, players: [], settings: const Settings(categoryIds: [], juniorMode: false, survivalMode: false), word: '', hostId: '', currentRound: 0, currentTurnIndex: 0, hostIsPremium: false, starterIndex: 0),
             playerId: 'p1',
        ));
        return setupCubit;
      },
      ...
      expect: () => [
        isA<SetupState>().having((s) => s.status, 'status', isA<SetupLoading>()),
        isA<SetupState>().having((s) => s.status, 'status', isA<SetupSuccess>()),
      ],
      verify: (_) {
        verify(() => mockRepo.joinGame(gameId: '1234', playerName: 'TestPlayer', avatarId: '')).called(1);
      },
    );
  });
}
